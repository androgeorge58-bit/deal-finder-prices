...
CHART_POINTS = 45       # downsample to this many points before shipping
FEED_LIMIT = 400        # how many products land in the app feed
...
def load_catalog():
    """Products discovered by the harvester (thousands), keyed by sku."""
...
def load_products():
    ...
    # harvested entries fill in anything the CSV does not cover; hand-written
    # rows win on conflict because you curated them deliberately.
    for sku, info in load_catalog().items():
        meta.setdefault(sku, info)
    return meta
...
def downsample(points, target):
    ...
    the first and last reading, plus the cheapest and dearest, so the chart
    never hides the very extremes the verdict is based on.
    """
    if len(points) <= target:
        return points
    keep = {0, len(points) - 1}
    keep.add(min(range(len(points)), key=lambda i: points[i]["price"]))
    keep.add(max(range(len(points)), key=lambda i: points[i]["price"]))
    step = len(points) / float(target - len(keep))
    i = 0.0
    while int(i) < len(points) and len(keep) < target:
        keep.add(int(i))
        i += step
...
def main():
    ...
    # FEED_LIMIT; the rest keep accumulating history and surface when they
    # actually become deals.
    ...
    if n:
        ...
        print(f"  NEW deals this run: {n}" + (f" (first 8: {shown})" if n else ""))
