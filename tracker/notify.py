...
# At catalogue scale a single run can turn 150+ products jade. Pushing all of
# them is how an app gets muted on day one. We push the few biggest drops and
# let the rest be discovered in the feed.
...
def main():
    ...
    strong = [d for d in queue if d["delta_pct"] >= MIN_PUSH_DELTA or d["verdict"] == "lowest"]
    skipped = len(queue) - len(strong)
    queue = sorted(strong, key=lambda d: -d["delta_pct"])[:MAX_PER_RUN]
    if skipped:
        print(f"{skipped} smaller drops held back from push (feed only)")
